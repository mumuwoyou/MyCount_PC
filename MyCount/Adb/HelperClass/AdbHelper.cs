...

{
    ...
    public class AdbHelper
    {
        ...
        /// <returns></returns> 
        public static bool SendBroadCastToDevice(string deviceNo, string action)
        {
            //adb shell am broadcast [options] <INTENT>
            var result = ProcessHelper.Run(AdbExePath, string.Format("-s {0} shell am broadcast -a {1}", deviceNo, action));
            m_log.Info("广播结果：" + result.ToString());
            if (!result.Success
                || result.ExitCode != 0
                || (result.OutputString != null && result.OutputString.Contains("failed")))
            {
                return false;
                throw new Exception("广播返回的结果异常：" + result.OutputString);
            }
            return true;
        }
        ...
        public static bool CopyToDevice(string deviceNo, string pcPath, string devPath)
        {
            ...
            m_log.Info("推送手机时结果：" + result.ToString());
                ...
                && result.OutputString.Contains("No such file or directory")))//若出现设备文件夹不存在的情况，则创建该文件夹 
            {
                ...
                */
                int index1 = devPath.IndexOf("/testdb.db");
                string devPath2 = devPath.Substring(0, index1);
                var moreArgs = new[] {"mkdir -p " + devPath2, "exit", "exit" };
                //shell 方式创建文件夹 
                result = ProcessHelper.Run(AdbExePath, string.Format("-s {0} shell mkdir -p {1}", deviceNo, devPath2));
                ...
                m_log.Info("再次推送手机结果：" + result.ToString());
